...
const DeliveryZonesSettings = ({
  ...
  const handleAddZone = () => {
    ...
    const previousMaxRadius =
      zones.length > 0 ? zones[zones.length - 1].maxRadius : 0
    setNewZone({ minRadius: previousMaxRadius, maxRadius: 0, price: 0 })
    setIsAdding(true)
  }

  const isZoneValid = (
    newZone: DeliveryZone,
  ...
  const handleRemoveZone = (index: number) => {
    if (index === 0 && zones.length > 1) {
      toast.error(
        'Pierwszej strefy nie można usunąć, dopóki istnieją inne strefy.'
      )
      return
    }
      ...
      {isAdding && newZone && (
        <div className="flex flex-col md:flex-row gap-4 w-3/4 justify-self-center pt-4">
          ...
          <div className="w-full">
            <Input
              ...
              onChange={(e) =>
                setNewZone({
                  ...newZone,
                  maxRadius: parseFloat(e.target.value) || NaN,
                })
              }
            />
          </div>
          ...
          <div className="flex self-center gap-2">
            ...
            <Button variant="secondary" size="sm" onClick={handleCancelNewZone}>
              Anuluj
            </Button>
          </div>
        </div>
